...
function exportPNG() {
  const W = 900, PAD = 40, SCALE = 2, LINE_H = 22, ROW_PAD = 10;
  const C = { bg: '#ffffff', text: '#141821', muted: '#6b7280',
              line: '#d7dbe3', head: '#eef1f6', accent: '#2f4b8f' };
  ...
  const date = $('#quoteDate').value.trim() || new Date().toISOString().slice(0, 10);
  ...
  rows.forEach(r => { h += r.lines.length * LINE_H + ROW_PAD; });
  h += 44;                                             // total
  ...
  if (forWho || byWho) {
    ...
    ctx.font = '13px "Segoe UI", sans-serif';
    ctx.fillStyle = C.muted;
    ctx.fillText([forWho && `For: ${forWho}`, byWho && `From: ${byWho}`]
      .filter(Boolean).join('     '), PAD, y);
  }
  y += 30;
  ...
  if (mbRows.length) {
    ...
    ctx.fillStyle = C.accent;
    ctx.font = '600 13px "Segoe UI", sans-serif';
    ctx.fillText(`FOR REFERENCE — CHEAPEST ON THE MARKET BOARD (${state.dc})`, PAD, y);
    y += 18;
  ...
  if (notesLines.length) {
    ...
    ctx.fillStyle = C.muted;
    ctx.font = '13px "Segoe UI", sans-serif';
    notesLines.forEach(l => { ctx.fillText(l, PAD, y); y += 18; });
  }
  ...
  ctx.fillStyle = C.muted;
  ctx.font = '11px "Segoe UI", sans-serif';
  ctx.fillText('Generated with the FFXIV Crafting Quote Builder', PAD, H - PAD + 6);

  canvas.toBlob(blob => {
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = (title.replace(/[^\w\-]+/g, '_').toLowerCase() || 'quote') + '.png';
    a.click();
