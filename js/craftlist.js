...
function leafCost(node) {
  if (node.children.length)
    return node.children.reduce((s, c) => s + leafCost(c), 0);
...
function nodeEl(node, isRoot, onRemove, rootUid) {
  ...
  if (cost && cost.filled) {
    ...
    const q = cost.quality
      ? `<em class="q ${cost.quality.toLowerCase()}">${cost.quality}</em> `
      : "";
...
function renderCraftList() {
  ...
  $("#refBuyTotal").textContent =
    gil(buyout) + " gil" + (buyoutPartial ? "*" : "");
  $("#refTotal").textContent =
    gil(materials) + " gil" + (matPartial ? "*" : "");
  ...
  price.innerHTML =
    cost && cost.filled
      ? `${cost.quality ? `<em class="q ${cost.quality.toLowerCase()}">${cost.quality}</em> ` : ""}${gil(cost.total)} <small>gil</small>` +
        (cost.short
          ? ` <small class="warn">(${cost.filled}/${cost.needed})</small>`
          : "")
      : "<small>no listing</small>";
